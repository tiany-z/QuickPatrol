...
import { handleClaimAppeal } from "./handler.js";
import { returnError } from "../../../../shared/flow/result.js";

export const api: ApiEndpointModule = {
  routePath: "/api/v4/feedback/appeals/claim",
  authRequired: true,
  handler: async (data: HttpRequestData, ctx: RequestContext) => {
    ...
    const departmentId = Number((ctx.userPayload as any)?.departmentId || (ctx as any).departmentId || 0);

    if (!schoolId || !userId) {
      return returnError("未授权的用户身份或租户标识丢失");
    }

    return handleClaimAppeal(
